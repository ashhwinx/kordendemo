...
const RollingText: React.FC<{ children: React.ReactNode, className?: string }> = ({ children, className = '' }) => {
  // Only animate if children is a string (to avoid breaking complex nodes)
  if (typeof children !== 'string') return <>{children}</>;

  return (
    <div className={`relative overflow-hidden flex flex-col items-center justify-center h-[1.2em] ${className}`}>
      {/* Original Text - Slides Up - Default Font Space Grotesk */}
      <span className="block transition-transform duration-500 ease-[cubic-bezier(0.2,0.8,0.2,1)] group-hover:-translate-y-[120%] font-['Space_Grotesk']">
        {children}
      </span>
      {/* Duplicate Text - Slides Up from Bottom - New Font Style Syne Italic */}
      <span className="absolute top-0 block translate-y-[120%] transition-transform duration-500 ease-[cubic-bezier(0.2,0.8,0.2,1)] group-hover:translate-y-0 text-amber-400 font-['Syne'] font-bold italic tracking-wide">
        {children}
      </span>
    </div>
  );
